...
const PUBLIC_FORMSUBMIT_ENDPOINT = `https://formsubmit.co/ajax/${PUBLIC_CONTACT_EMAIL}`;
...
async function sendFormSubmitFallback(payload) {
  ...
  const response = await fetch(PUBLIC_FORMSUBMIT_ENDPOINT, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || responseBody?.success === "false") {
    throw new Error(
      responseBody?.message || "Unable to submit consultation request."
    );
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      const alertResult = await sendResendAlert(body);
      if (!alertResult.configured) {
        await sendFormSubmitFallback(body);
      }
    } catch (alertError) {
      console.error("Cedar consultation alert failed, falling back.", alertError);
      await sendFormSubmitFallback(body);
    }
    ...
    try {
      ...
      console.error("Cedar onboarding registration failed.", onboardingError);
    }
