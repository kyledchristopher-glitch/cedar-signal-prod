...
const PortfolioCard = ({ project, index }: { project: (typeof projects)[number]; index: number }) => {
  ...
  const y = useTransform(
    scrollYProgress,
    [0, 1],
    prefersReducedMotion ? [0, 0] : [18 - index * 4, -14 + index * 3]
  );
  ...
  return (
    <motion.article
      ...
      className="card-glow group flex h-full flex-col overflow-hidden rounded-[30px] border border-brass/14 bg-gradient-to-b from-bark/18 via-background/90 to-background shadow-[0_28px_80px_hsl(var(--background)/0.26)]"
    >
      <a
        href={project.href}
        target="_blank"
      ...
      >
        <div className="overflow-hidden rounded-[24px] border border-brass/10 bg-background/80 shadow-[inset_0_1px_0_hsl(var(--parchment)/0.05)]">
          ...
          <div className="relative bg-background">
            <div className="pointer-events-none absolute inset-0 z-10 bg-gradient-to-t from-background/18 via-transparent to-parchment/5" />
            <img
              src={project.preview}
              alt={project.previewAlt}
              className="aspect-[16/12] w-full object-cover object-top transition-transform duration-700 ease-out group-hover:scale-[1.06]"
              loading="lazy"
              width={1005}
              height={768}
            />
      ...
      <div className="flex flex-1 flex-col px-6 pb-7 pt-1 md:px-7">
        ...
        <h3 className="font-serif text-[1.9rem] leading-none text-foreground md:text-[2.15rem]">
          {project.title}
        </h3>
...
const PortfolioSection = () => {
  return (
    <section id="portfolio" className="section-blend relative py-24 md:py-32">
